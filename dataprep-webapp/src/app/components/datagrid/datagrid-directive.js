			...
			const onFiltersChange = function onFiltersChange(filterConfig) {
				if (grid) {
					...
					const {
						dataModel,
						nbTotalLines,
					} = state.playground.grid;

					dataModel.setFilters(
						enabled ? filters : null
					);

					dataModel.setLength(nbTotalLines);
					...
					if (getSelectedColumns().length === 1) {
						...
						if (stateSelectedLine && stateGridData && !stateGridData.preview) {
							cellHighlightTimeout = $timeout(
								() => {
									const { id } = stateSelectedColumn;
									const content = stateSelectedLine[id];
									DatagridStyleService.highlightCellsContaining(id, content);
								},
								500,
								false
							);
