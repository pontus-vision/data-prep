...
public final class TransformationContext implements Serializable {
    ...
    private final Map<RunnableAction, ActionContext> contexts;
    ...
    public void cleanup() {
        ...
        for (ActionContext currentContext : allActionsContexts) {
            new HashSet<>(currentContext.getContextEntries()).forEach(contextEntry -> {
                try {
                    try {
        ...
        for (Object o : context.values()) {
            if(o instanceof Closeable) {
                try {
                    ...
                    LOGGER.warn("error cleaning object from context {}", e);
                }
            }
        }
    }
     ...
     */
    public ActionContext create(RunnableAction action, RowMetadata rowMetadata) {
        return create(action, rowMetadata, Collections.emptyMap());
    }
     ...
     */
    public ActionContext create(RunnableAction action, RowMetadata rowMetadata, Map<String, String> parameters) {
     ...
     */
    public ActionContext in(RunnableAction action) {
        if (contexts.containsKey(action)) {
            return contexts.get(action);
        } else {
