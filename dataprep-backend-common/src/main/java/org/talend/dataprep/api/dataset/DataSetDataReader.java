 ...
 * </pre>
 * 
 * It will be read as stream by first reading metadata to extract columns metadata in the form of {@link RowMetadata}
 * then,
 * records will be read and transformed into {@link DataSetRow DataSetRows}.
 * <p>
 * <strong>Warning</strong> This class is strongly relying on exception to work: {@link IOException} and
...
public final class DataSetDataReader {
    ...
    private static final String INCORRECT_OBJECT_STRUCTURE_ERROR_MESSAGE =
            "Malformed lookup data from DataPrep server.";
    ...
    public LightweightExportableDataSet parseAndMapLookupDataSet(InputStream inputStream, String joinOnColumn)
        ...
        try (JsonParser jsonParser =
                mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false).getFactory().createParser(
    ...
    private Map<String, Map<String, String>> parseRecords(JsonParser jsonParser, RowMetadata rowMetadata,
        ...
        try {
            ...
            Iterator<Map<String, String>> mapIterator =
                    jsonParser.readValuesAs(new TypeReference<Map<String, String>>() {
                    });
