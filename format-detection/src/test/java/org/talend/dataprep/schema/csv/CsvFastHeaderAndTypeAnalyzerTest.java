...
public class CsvFastHeaderAndTypeAnalyzerTest {
    ...
    public void should_neither_detect_type_nor_first_line_header_when_sample_is_empty() {
        ...
        Assert.assertArrayEquals(expectedTypes.toArray(), analysis.getHeaders().stream().map(Pair::getValue).toArray());
        Assert.assertFalse(analysis.isFirstLineAHeader());
    }

    @Test
    @Ignore("We do not care about the types")
    ...
    public void TDP_289_first_case() {
        ...
        Assert.assertArrayEquals(expectedHeaders.toArray(), analysis.getHeaders().stream().map(Pair::getKey).toArray());
        Assert.assertArrayEquals(expectedTypes.toArray(), analysis.getHeaders().stream().map(Pair::getValue).toArray());
        Assert.assertFalse(analysis.isFirstLineAHeader());
    }
