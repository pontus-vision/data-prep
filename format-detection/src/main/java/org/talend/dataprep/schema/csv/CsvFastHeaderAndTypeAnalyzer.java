...
class CsvFastHeaderAndTypeAnalyzer {
    ...
    public CsvFastHeaderAndTypeAnalyzer(List<String> sampleLines, Separator separator) {
        if (sampleLines == null || sampleLines.isEmpty()){
            throw  new IllegalArgumentException("The sample used for analysis must kneither be null nor empty!");
        }
        if ( separator == null){
        ...
        if (sampleLines.size() > 1) {
            ...
            if (firstLineAHeader) {
                ...
                for(String field: firstLine) {
                    headers.add(new ImmutablePair<>(field, columnTypes.get(i++)));
                }
            } else {
                List<Type> columnTypes = allRecordsColumnTyping();
                int i = 1;
                for (Type type : columnTypes) {
                    headers.add(new ImmutablePair<>("COL" + i++, type));
                }
            }
