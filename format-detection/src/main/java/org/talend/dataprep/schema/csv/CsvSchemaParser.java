...
public class CsvSchemaParser implements SchemaParser {
    ...
    public String getDefaultTextEnclosure() {
        return csvFormatUtils.getDefaultTextEnclosure();
    }

    public void setDefaultTextEnclosure(String defaultTextEnclosure) {
        csvFormatUtils.setDefaultTextEnclosure(defaultTextEnclosure);
    }

    public String getDefaultEscapeChar() {
        return csvFormatUtils.getDefaultEscapeChar();
    }

    public void setDefaultEscapeChar(String defaultEscapeChar) {
        csvFormatUtils.setDefaultEscapeChar(defaultEscapeChar);
    }
    ...
    public List<SheetContent> parse(Request request) {
        List<SheetContent> sheetContents = new ArrayList<>();
        SheetContent sheetContent = new SheetContent(META_KEY, new ArrayList<>());
        sheetContents.add(sheetContent);
        try {
            final Map<String, String> parameters = guess(request.getContent(), request.getEncoding(), request.getParameters());
            ...
            for (String column : header) {
                sheetContent.getColumnMetadatas() //
                        .add(SheetContent.ColumnMetadata.Builder.column().id(i++).name(column).build());
            }
        } catch (Exception e) {
            throw new TalendRuntimeException(CommonErrorCodes.UNABLE_TO_READ_CONTENT, e);
        }
     ...
     */
    public Map<String, String> guess(InputStream content, Charset encoding, Map<String, String> parameters) {
        ...
        final String temp = parameters.get(CsvFormatFamily.SEPARATOR_PARAMETER);
        if (temp != null && StringUtils.isNotEmpty(temp)) {
            forcedSeparator = Optional.of(temp.charAt(0));
        }

        Separator sep = guessSeparator(content, encoding, forcedSeparator);

        return csvFormatUtils.compileParameterProperties(sep);
    }
     ...
     */
    private Separator guessSeparator(InputStream is, Charset encoding, Optional<Character> forcedSeparator) {
        try (CSVStreamReader csvStreamReader = new CSVStreamReader(is, encoding, SIZE_LIMIT, LINE_LIMIT)) {
            ...
            final List<Character> validSeparators;

            validSeparators = forcedSeparator.map(Collections::singletonList).orElse(DEFAULT_VALID_SEPARATORS);

            while ((line = csvStreamReader.readLine()) != null) {
                if (!line.isEmpty() && sampleLines.size() < SMALL_SAMPLE_LIMIT) {
                    sampleLines.add(line);
                }
                processLine(line, separatorMap, validSeparators, csvStreamReader.getLineCount());
            }
    ...
    class CSVStreamReader implements AutoCloseable {
         ...
         */
        public CSVStreamReader(InputStream inputStream, Charset encoding, long sizeLimit, int lineLimit) {
